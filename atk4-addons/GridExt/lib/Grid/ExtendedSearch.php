...
class Grid_ExtendedSearch extends \Filter {
    ...
    function init() {
        ...
        $this->bt = $this->owner->addButton('')
                //->addClass('float-right')
                ->setHtml('&nbsp;')
                ->setAttr('title', 'Advance Search')
                ->addClass('extended-search-button');
        
        // add icon to butotn 
        $opt['icons']['primary']='ui-icon-search';
        $this->bt->js(true)->button($opt);
        
        $this->bt->js('click', $this->js()->toggle());

        // Form styling
//        $this->addClass('float-right stacked');
        $this->addClass('atk-row');
    ...
    function submitted() {
        if ($_GET['submit']==$this->name) {
            if ($this->isClicked($this->bs)) {
                $this->memorize($this->token, true);
            }
            if ($this->isClicked($this->bc)) {
                $this->forget($this->token);
